...
amean <- function(x, na.rm = TRUE) {
  ...
  res <- ifelse((length(x) > 0) , sum(x) / length(x), NA)
...
asd <- function(x, na.rm = TRUE) {
  ...
  mu  <- amean(x)
  res <- ifelse((length(x) > 0) , sqrt( sum((x - mu) ^ 2) / length(x) ), NA)
...
wamean <- function(x, w, na.rm = TRUE) {
  ...
  res <- ifelse((length(x) > 0), sum(w * x) / sum(w), NA)
...
#  return( ifelse(length(x), prod(x) ^ (1/length(x)), NA) )
  res <- ifelse((length(x) > 0) , exp(amean(log(x))), NA)
...
wgmean <- function(x, w, na.rm = TRUE) {
  ...
  res <- ifelse((length(x) > 0) , length(x) / sum(1/x), NA)
...
whmean <- function(x, w, na.rm = TRUE) {
  ...
  res <- ifelse((length(x) > 0) , sum(w) / sum(w/x), NA)
...
mse <- function(pred, obs, na.rm = TRUE)
{
  ...
  res <- ifelse(length(obs), sum((pred - obs) ^ 2) / length(obs), NA)
...
R2mse <- function(pred, obs, na.rm = TRUE) {
  ...
  if (length(obs) > 1) {
    ...
    rss   <- sum((obs - pred) ^ 2)
    res   <- ifelse( (tss > 0) , (tss - rss) / tss, NA)
  }
...
R2toCVmse <- function(R2, obs) {
  ...
  res <- sqrt( (1 - R2) * var2(obs) ) / amean(obs)
...
test.posthoc <- function(x, clusters, pvalue = 0.01) {
  ...
  if (length(index) > 1) {
    ...
    res    <- data.frame(motifs, means, sds, groups, mlet)
    colnames(res) <- c("motif", "mean", "sd", "group", colnames(mlet))
  }
