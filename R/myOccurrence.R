...
segment.vector <- function(v, nbSeg,
  ...
  sv.median  <- cumsum(sw)
    ...
    # option = "amean"
  sv.amean   <- cumsum(sv$x)

  # option = "gmean"
  sv.gmean   <- c(rep(0, sum(sv$x == 0)), cumsum(log(sv$x[sv$x != 0])))
...
#xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx
#
# plot the sorted presence frequency

#xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx

plot.effect <- function(fct, mOccur,
                        titre = "effect",
                        order = c(1:dim(mOccur)[2]),
                        windw = c(1:dim(mOccur)[2]),
                        cols  = rep(couleurs[1], length(order)),
  ...
  fctElt <- numeric(nbElt)
  for (elt in seq_len(nbElt))
    fctElt[elt] <- mean(fct[which(mOccur[ , elt] == 1)])
  names(fctElt) <- colnames(mOccur)

  plot.fofelements(v = fctElt, ytitre = titre,
                   index = order, wdw = windw,
                   cols = cols, figs = figs)
}
...
plot.occurring <- function(nbOpt, xwdw = c(1:50),
  ...
  # plot the mean effect induced by each element
  #   sorted according to tree clustering
  #       cluster by cluster
  # plot the element abundaces sorted by median values

  Fobs <- res.prd$fct

  plot.effect(Fobs, mOccur,
              titre = "mean Fobs of assembly set containing an element",
              order = ind.abund)

  #   sorted according to tree clustering
  #       cluster by cluster
  plot.effect(Fobs, mOccur.split,
              titre = "mean Fobs of assembly set containing an element",
              order = ind.tree, cols  = col.tree, figs  = fig.tree)
  ...
  # plot the sorted abundance  Utile ici ????
  fctElt <- numeric(length(Fobs))
  for (elt in seq_len(nbElt.split))
    fctElt[elt] <- mean(Fobs[which(mOccur.split[ , elt] == 1)])

  plot.box.elements(Fobs, mOccur.split,
                    colnames(mOccur.split)[ind.tree], ind.tree,
                    ylim     = range(Fobs),
                    titre = "mean Fobs of assembly set containing an element",
...
vfreq <- apply(mOccur.split, MARGIN = 2, FUN = sum) / dim(mOccur.split)[1]
hist(vfreq, breaks = 30)

vfreq <- apply(mAbund.split, MARGIN = 2, FUN = sum) / dim(mAbund.split)[1]
hist(vfreq, breaks = 30)
...
if (auPoint == TRUE) {
  ...
  for (nbcl in 1:length(unique(affectElt))) {
    ...
    for (elt in 1:length(doublon)) {
      ...
      nbLev  <- nbLev.split[which(names(nbLev.split) == names(doublon)[elt])]
...
#oooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooo
# Get together the abundance an,d occseconde version
...
sort.split.names <- function(affectElt, ind.tree,
  ...
  s.short.names <- unique(setRad[ind.tree])

  index <- integer(length(short.names))
  for (elt in seq_along(s.short.names))
    index[elt] <- which(short.names == s.short.names[elt])
