...
#xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx
#
#  Expand a tree by inserting a new element
#
#  Input:  tree    : to be expanded
#          nbOpt   : where element is considered similar to an other
#          newElt  : name of the new element
#          sameElt : name of an existing element assumed close to newElt
#          sameFG  : letter of functional group
#                           of which the newElt is assumed belonging
#  Output: an expanded tree including the new element
#
#xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx

expand.tree <- function(tree, nbOpt, newElt, sameElt = "", sameFG = "") {
  ...
  if (sameElt == "") {
    ...
    index0  <- ifelse( sameFG == "" , 1, 1 + which(letters == sameFG) )
    index0  <- ifelse( index0 > nbOpt ,  index0, 1 )
    index1  <- which(expandAff[nbOpt, ] == index0)
    index2  <- which.max(expandAff[nbElt, index1])
    sameElt <- names(index2)
  }

  newCol   <- expandAff[ , sameElt]
  newRow    <- c(expandAff[nbElt, ], nbElt + 1)

  expandAff <- cbind(expandAff, newCol)
  expandAff <- rbind(expandAff, newRow)
  colnames(expandAff)[nbElt + 1] <- newElt
  rownames(expandAff)[nbElt + 1] <- nbElt + 1

  expandCor <- c(tree$cor, tree$cor[nbElt])
  names(expandCor) <- seq_len(nbElt + 1)

  res        <- list(expandAff, expandCor)
  names(res) <- c("aff", "cor")
...
reduce.tree <- function(tree, oldElt) {

  nbElt  <- dim(tree$aff)[1]

  newCol <- tree$aff[ , sameElt]
  newRow <- c(tree$aff[nbElt, ], nbElt + 1)

  expandAff <- cbind(tree$aff, newCol)
  expandAff <- rbind(expandAff, newRow)
  colnames(expandAff)[nbElt + 1] <- newElt
  rownames(expandAff)[nbElt + 1] <- nbElt + 1
